...
public class PlayerService extends Service{
	...
	{
		Log.i(ListenToMusicApp.TAG, "Player Service onCreate");
		...
		mPhoneStateListener = new PhoneStateListener(){
			...
			{
				Log.e(ListenToMusicApp.TAG, "onCallStateChanged");
		...
		mWifiLock = mWifiManager.createWifiLock(ListenToMusicApp.TAG);
		mWifiLock.setReferenceCounted(false);
		
		ListenToMusicApp.getInstance().setConcretePlayerEngine(mPlayerEngine);
		mRemoteEngineListener = ListenToMusicApp.getInstance().fetchPlayerEngineListener();
	}
	...
	public void onStart(Intent intent, int startId) {
		...
		Log.i(ListenToMusicApp.TAG, "Player Service onStart - "+action);
	...
	private void updatePlaylist(){
		if(mPlayerEngine.getPlaylist() != ListenToMusicApp.getInstance().fetchPlaylist()){
			mPlayerEngine.openPlaylist(ListenToMusicApp.getInstance().fetchPlaylist());
		}
	}
	
	@Override
	public void onDestroy() {
		Log.i(ListenToMusicApp.TAG, "Player Service onDestroy");
		ListenToMusicApp.getInstance().setConcretePlayerEngine(null);
		mPlayerEngine.stop();
		mPlayerEngine = null;
		// unregister listener
	...
	private void scrobblerMetaChanged() {
		...
		if (entry != null) {
			...
			if (Log.isLoggable(ListenToMusicApp.TAG, Log.INFO)) {
				Log.i(ListenToMusicApp.TAG, "Scrobbling track " + entry.title + " via " + scrobblerApp);
			}
			
			if (scrobblerApp.equalsIgnoreCase("lastfm")) {
				Intent i = new Intent(LASTFM_INTENT);
				i.putExtra("artist", entry.artist);
				i.putExtra("album", entry.album);
				i.putExtra("track", entry.title);
				i.putExtra("duration", entry.duration); // duration in milliseconds
				sendBroadcast(i);
				...
				i.putExtra("artist", entry.artist);
				i.putExtra("track", entry.title);
				i.putExtra("duration", entry.duration); // duration in seconds
				i.putExtra("album", entry.album);
				i.putExtra("track-no", entry.albumId);
				sendBroadcast(i);
			} else {
	...
	private void displayNotifcation(PlaylistEntry playlistEntry)
	{
		...
		String notificationMessage = playlistEntry.title + " - " + playlistEntry.artist;

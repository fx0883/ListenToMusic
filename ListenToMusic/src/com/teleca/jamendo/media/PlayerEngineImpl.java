...
public class PlayerEngineImpl implements PlayerEngine {
	...
	public void play() {
		...
		if(mPlaylist != null){
			...
			if(!mCurrentMediaPlayer.preparing){
				...
				if(!mCurrentMediaPlayer.isPlaying()){
					...
					Log.i(ListenToMusicApp.TAG, "Player [playing] "+mCurrentMediaPlayer.playlistEntry.title);
					
					final ListenToMusicApp app = ListenToMusicApp.getInstance();
                    ...
                    ListenToMusicApp.getInstance().setMyEqualizer(newEqualizer);
                    ListenToMusicApp.getInstance().getMyEqualizer().setEnabled(true);
		...
		ListenToMusicApp.getInstance().setMyCurrentMedia(mCurrentMediaPlayer);
	}
	...
	private void cleanUp(){
		...
		if(mCurrentMediaPlayer != null) {
			...
			Equalizer eq = ListenToMusicApp.getInstance().getMyEqualizer();
			if (eq != null) {
				eq.release();
				ListenToMusicApp.getInstance().setMyEqualizer(null);
			}
		}
	}
...
//		String path = ListenToMusicApp.getInstance().getDownloadManager().getTrackPath(playlistEntry);
		String path = null;
		if(path == null)
			// fallback to remote one
			path = playlistEntry.sourceUrlStr;
...
//			
			...
			mediaPlayer.setOnErrorListener(new OnErrorListener() {
				...
				public boolean onError(MediaPlayer mp, int what, int extra) {
					Log.w(ListenToMusicApp.TAG, "PlayerEngineImpl fail, what ("+what+") extra ("+extra+")");
					...
					if(what == -1){
						...
						if(failTime - mLastFailTime > FAIL_TIME_FRAME){
							...
							Log.w(ListenToMusicApp.TAG, "PlayerEngineImpl "+mTimesFailed+" fail within FAIL_TIME_FRAME");
						} else {
							// inside time frame
							mTimesFailed++;
							if(mTimesFailed > ACCEPTABLE_FAIL_NUMBER){
								Log.w(ListenToMusicApp.TAG, "PlayerEngineImpl too many fails, aborting playback");
								if(mPlayerEngineListener != null){
									mPlayerEngineListener.onTrackStreamError();
								}
								stop();
			...
			// start preparing
			Log.i(ListenToMusicApp.TAG, "Player [buffering] "+mediaPlayer.playlistEntry.title);
